...
files immediately after the container starts. Keep generated outputs under
`/workspace/wifi-log-analyzer` so they survive pod restarts. Mounting a RunPod
volume at `/workspace` can hide files that were baked into the image under
`/workspace`, so the Docker image keeps code and bundled inputs outside that
mount.

For readable inputs, relative paths are checked in this order:
...
For inference models, relative `model_dir` values are checked in this order:
